...
*(Просто напиши там «**Жмых**» или ответь реплаем на любое мое сообщение)*
        ...
        ai.analyzeUserImmediate(contextForAnalysis, userProfile).then(updated => {
            if (updated) {
                if (updated.relationship) {
                    // console.log(`[RELATIONSHIP] ${senderName}: ${updated.relationship}`);
                }
                const updates = {};
                if (userId === config.adminId) {
                    updated.relationship = 100;
                    updated.attitude = "Создатель. Батя.";
                }
                updates[userId] = updated;
                storage.bulkUpdateProfiles(chatId, updates);
            }
